...
const ManageJobs: React.FC = () => {
  ...
  // Mock data
  useEffect(() => {
    console.log('📋 Loading mock jobs data...');
    const mockJobs: Job[] = [
      {
        id: "1",
  ...
  const handleEditJob = (jobId: string) => {
    console.log('✏️ Editing job with ID:', jobId);
    
    // Check if job exists in current data
    const jobExists = jobs.find(job => job.id === jobId);
    if (!jobExists) {
      console.error('❌ Job not found for editing:', jobId);
      alert('Job not found. This job may have been deleted or is no longer available.');
      return;
    }
    
    console.log('✅ Job found for editing:', jobExists.title);
  ...
  const handleViewJob = (jobId: string) => {
    ...
    const jobExists = jobs.find(job => job.id === jobId);
    if (!jobExists) {
      console.error('❌ Job not found in current data:', jobId);
      alert('Job details not found. This job may have been deleted or is no longer available.');
      return;
    }
    
    console.log('✅ Job found, navigating to details:', jobExists.title);
