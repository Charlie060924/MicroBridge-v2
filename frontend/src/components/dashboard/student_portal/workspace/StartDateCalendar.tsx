...
const StartDateCalendar: React.FC<StartDateCalendarProps> = ({
  ...
  const getDayClassNames = (day: CalendarDay): string => {
    const baseClasses = `
      ...
      ${!day.isCurrentMonth ? 'text-gray-400' : 'text-gray-900 dark:text-white'}
    ...
    if (!day.isCurrentMonth || day.isPast || day.isUnavailable) {
      return `${baseClasses} text-gray-400 cursor-not-allowed hover:bg-gray-100 dark:hover:bg-gray-800`;
    }
    ...
    if (day.isInRange) {
      return `${baseClasses} bg-blue-100 text-blue-900 dark:bg-blue-900/30 dark:text-blue-100`;
    }
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <button
        ...
        disabled={disabled}
        className={`
          w-full flex items-center justify-between px-4 py-3 text-left bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm
          ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:border-gray-400 dark:hover:border-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent'}
          transition-all duration-200
        `}
      >
        <div className="flex items-center gap-3">
          <CalendarIcon className="h-5 w-5 text-gray-400" />
          <span className={selectedDate || selectedStartDate ? 'text-gray-900 dark:text-white' : 'text-gray-500 dark:text-gray-400'}>
            {getDisplayText()}
          </span>
        </div>
      ...
      {/* Calendar Dropdown */}
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-50">
          {/* Quick Selection */}
            ...
            <div className="grid grid-cols-2 gap-2">
              {quickOptions.map((option) => (
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 rounded-md hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"
                >
                  {option.icon}
                  {option.label}
                </button>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-1 text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-colors"
                >
                  <ChevronLeft className="h-4 w-4" />
                </button>
                <button
                  onClick={goToToday}
                  className="px-2 py-1 text-xs bg-blue-100 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded hover:bg-blue-200 dark:hover:bg-blue-900/40 transition-colors"
                >
                  Today
                </button>
                <button
