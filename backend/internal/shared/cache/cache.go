...
type LogEvent interface {
	...
	Strs(key string, values []string) LogEvent
	Msg(msg string)
}
...
func (l *defaultLogger) Info() LogEvent {
	return &defaultLoggerEvent{} 
}

func (l *defaultLogger) Error() LogEvent {
	return &defaultLoggerEvent{} 
}

type defaultLoggerEvent struct{}

func (e *defaultLoggerEvent) Str(key, value string) LogEvent { return e }
func (e *defaultLoggerEvent) Err(err error) LogEvent { return e }
func (e *defaultLoggerEvent) Strs(key string, values []string) LogEvent { return e }
...
// MatchScore represents a generic match score for caching
...
func (c *CacheLayer) GetMatchResult(ctx context.Context, userID, jobID string) (*MatchScore, error) {
	key := fmt.Sprintf("match:%s:%s", userID, jobID)
	
	// Try L1 cache first
	if cached, ok := c.L1.Load(key); ok {
		if cache, ok := cached.(*MatchScore); ok {
...
func (c *CacheLayer) SetMatchResult(ctx context.Context, userID, jobID string, matchScore *MatchScore) error {
	key := fmt.Sprintf("match:%s:%s", userID, jobID)
