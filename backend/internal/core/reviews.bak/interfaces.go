...
type AuthService interface {
	GenerateTokens(ctx context.Context, userID string) (accessToken string, refreshToken string, expiresAt time.Time, err error)
	ValidateToken(ctx context.Context, tokenString string) (*dto.TokenClaims, error)
	RefreshToken(ctx context.Context, refreshToken string) (accessToken string, expiresAt time.Time, err error)
